...
def a_star_dict(nodes_dict, start_node, end_node):
    try:
        if start_node not in nodes_dict.keys() or end_node not in nodes_dict.keys():
    ...
    unknown_nodes = {k: [np.inf, []] for k in nodes_dict.keys() if
                     k != start_node}  # nodes we don't know the path to
    for k in nodes_dict[start_node].keys():
        unknown_nodes[k] = [nodes_dict[start_node][k] + heuristic(k, end_node), [start_node]]
    ...
    while any(np.isfinite([unknown_nodes[k][0] for k in unknown_nodes.keys()])):
        ...
        for successor in u_successors:
            if successor in unknown_nodes.keys() and unknown_nodes[successor][0] > unknown_nodes[u][0] - heuristic(u,
                                                                                                                   end_node) + \
                    nodes_dict[u][successor] + heuristic(successor, end_node):
                unknown_nodes[successor][0] = unknown_nodes[u][0] + nodes_dict[u][successor] - heuristic(u,
                                                                                                         end_node) + heuristic(
                    successor, end_node)
...
def a_star_dict_visualizer(nodes_dict, start_node, end_node, blockSize, screen):
    Orange = (255, 215, 0)
    BLUE = (0, 0, 255)
    try:
        if start_node not in nodes_dict.keys() or end_node not in nodes_dict.keys():
