...
def main():
    ...
    while start_button == 0:
        for event in pygame.event.get():
            ...
            if button_down != 0:
                if button_down == 1:
                    if pygame.mouse.get_pos()[0] > WINDOW_WIDTH - 150 and pygame.mouse.get_pos()[
                        0] < WINDOW_WIDTH - 50 and pygame.mouse.get_pos()[1] > 375 and pygame.mouse.get_pos()[1] < 425:
                        if start_node is not None and end_node is not None and pressed_algo is not None:
                            ...
                            screen.blit(img, (WINDOW_WIDTH - 125, 390))
                    if pygame.mouse.get_pos()[0] > 85 and pygame.mouse.get_pos()[0] < 115 and pygame.mouse.get_pos()[
                        1] > 200 and pygame.mouse.get_pos()[1] < 230:
                        if pressed_algo != 'Dijkstra':
                            ...
                            pygame.draw.rect(screen, BLACK, rect, 1)
                    if pygame.mouse.get_pos()[0] > 85 and pygame.mouse.get_pos()[0] < 115 and pygame.mouse.get_pos()[
                        1] > 600 and pygame.mouse.get_pos()[1] < 630:
                        if pressed_algo != 'A*':
                            pressed_algo = 'A*'
                            ...
                            pygame.draw.rect(screen, WHITE, rect, 0)
                            pygame.draw.rect(screen, BLACK, rect, 1)
                    if pygame.mouse.get_pos()[0] > 200 and pygame.mouse.get_pos()[0] < WINDOW_WIDTH - 200 and \
                            pygame.mouse.get_pos()[1] > 0 and pygame.mouse.get_pos()[1] < WINDOW_HEIGHT:
                        if start_node is not None:
                            rect = pygame.Rect(int(start_node[2:]) * blockSize + 200, int(start_node[:2]) * blockSize,
                                               blockSize, blockSize)
                            pygame.draw.rect(screen, WHITE, rect, 0)
                            pygame.draw.rect(screen, BLACK, rect, 1)
                        start_node = put_zeros_before_int(
                            pygame.mouse.get_pos()[1] // blockSize) + put_zeros_before_int(
                            (pygame.mouse.get_pos()[0] - 200) // blockSize)
                        rect = pygame.Rect(int(start_node[2:]) * blockSize + 200, int(start_node[:2]) * blockSize,
                                           blockSize, blockSize)
                ...
                if button_down == 2:
                    if pygame.mouse.get_pos()[0] > 200 and pygame.mouse.get_pos()[0] < WINDOW_WIDTH - 200 and \
                        ...
                        if end_node is not None:
                            rect = pygame.Rect(int(end_node[2:]) * blockSize + 200, int(end_node[:2]) * blockSize,
                                               blockSize, blockSize)
                            pygame.draw.rect(screen, WHITE, rect, 0)
                            pygame.draw.rect(screen, BLACK, rect, 1)
                        end_node = put_zeros_before_int(pygame.mouse.get_pos()[1] // blockSize) + put_zeros_before_int(
                            (pygame.mouse.get_pos()[0] - 200) // blockSize)
                        rect = pygame.Rect(int(end_node[2:]) * blockSize + 200, int(end_node[:2]) * blockSize,
                                           blockSize, blockSize)
                        pygame.draw.rect(screen, GREEN, rect, 0)
                ...
                if button_down == 3:
                    if pygame.mouse.get_pos()[0] > 200 and pygame.mouse.get_pos()[0] < WINDOW_WIDTH - 200 and \
                            pygame.mouse.get_pos()[1] > 0 and pygame.mouse.get_pos()[1] < WINDOW_HEIGHT:
                        clicked_node = put_zeros_before_int(
                            pygame.mouse.get_pos()[1] // blockSize) + put_zeros_before_int(
                            (pygame.mouse.get_pos()[0] - 200) // blockSize)
                        if clicked_node not in blocked_nodes:
                            blocked_nodes.append(clicked_node)
                            rect = pygame.Rect(int(clicked_node[2:]) * blockSize + 200,
                                               int(clicked_node[:2]) * blockSize, blockSize, blockSize)
